...
from src.email_assistant.tools.gmail.run_ingest import fetch_and_process_emails


@dataclass(kw_only=True)
class JobKickoff:
    """State for the email ingestion cron job"""
    email: str ="chaitanyabj@gmail.com"
    minutes_since: int = 60
    graph_name: str = "email_assistant_gmail"
...
# Build the graph
graph_workflow = StateGraph(JobKickoff)
graph_workflow.add_node("ingest_emails", main)
graph_workflow.set_entry_point("ingest_emails")
graph_workflow.add_edge("ingest_emails",END)
graph = graph_workflow.compile()
print("Cron graph compiled")
